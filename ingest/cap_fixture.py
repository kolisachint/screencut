...
def write_bundle(
    ...
    *,
    ...
    # This bundle is in Cap's own format and is read by the real adapter, so it is
    # indistinguishable from a recording by design. The marker is how it stays
    # honest about that where it matters — §10.2 counts real takes only
    # (`ingest/cap.py`).
    (root / SYNTHETIC_MARKER).write_text(
        "Generated by ingest/cap_fixture.py. Not a recording; see spec/source.py.\n"
    )
